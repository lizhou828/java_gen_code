...
//	Properties props;
	...
	public Table getTable(String sqlTableName) throws Exception {
		...
		while(rs.next()) {
			...
			if(table != null && StringHelper.isEmpty(table.getRemarks())){
				/* 如果没有获取到表的注释，则用建表语句中，找到表注释 */
				table.setRemarks(getCommentByTableName(sqlTableName));
			}
			return table;
		}
	 ...
	 * @throws Exception
	 */
	private String getCommentByTableName(String sqlTableName) throws Exception {
		Connection conn = getConnection();
		Statement stmt = conn.createStatement();
		ResultSet rs = stmt.executeQuery("SHOW CREATE TABLE " + sqlTableName);
		String comment = "";
		if (rs != null && rs.next()) {
			String createDDL = rs.getString(2);
			comment = parseTableComment(createDDL);
		}
		rs.close();
		stmt.close();
		conn.close();
